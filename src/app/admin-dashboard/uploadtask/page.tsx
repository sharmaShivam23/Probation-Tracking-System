...
export default function TaskForm() {
  ...
  const validateField = (name: string, value: string) => {
    ...
    if (!value.trim() && ["title", "description", "link", "deadline", "category"].includes(name)) {
    ...
    if (name === "title") {
      if (!value.trim()) errorMsg = "Title is required";
        ...
        errorMsg = "Only letters, numbers, spaces, and '-' allowed";
    }
     if (name === "description") {
      if (!value.trim()) errorMsg = "Description is required";
      else if (!/^[a-zA-Z0-9\-,._\s]+$/.test(value))
        errorMsg = "Only letters, numbers, spaces, '-', '.', '_' allowed";
    }
     if (name === "link" && value.trim()) {
      // ✅ Google Drive link validation
      const driveRegex =
        /^(https?:\/\/)?(www\.)?drive\.google\.com\/(file\/d\/|open\?id=)[\w-]+/;
      if (!driveRegex.test(value)) errorMsg = "Enter a valid Google Drive link";
    }
  ...
  const handleVerifyClick = () => {
    ...
    Object.keys(formData).forEach((key) => {
      if (["title", "description","link", "deadline", "category"].includes(key)) {
